...
namespace Utilities
{
	...
	static constexpr StringHash s_PrimeTable[] = {
	...
	constexpr StringHash hashString( const char* toHash, size_t size )
	{
		StringHash hash = 0x811c9dc5;
		for ( size_t i = 0; i < size; ++i )
		{
			uint8_t value = toHash[i];
			hash = hash ^ value;
			hash = StringHash( uint64_t( hash ) * (s_PrimeTable[(static_cast<size_t>(toHash[i]) << 5) % s_PrimeNumCount]) );
		}
		return hash;
	}
	template<size_t N>
	...
	public:
		...
		{};
		constexpr StringHash hash()
		{
			return hashString( string, N - 1 );
		}
		constexpr char operator[]( std::size_t n ) const
		{
			return (n >= N - 1) ? '\0' : string[n];
		};
	...
	template<size_t N>
	constexpr StringHash hashString( const ConstexprString<N>& toHash )
	{
		return hashString( toHash, static_cast<uint32_t>(toHash.size()) );
	}
	template<size_t N>
	constexpr StringHash hashString( const char( &a )[N] )
	{
		return hashString( a, N - 1 );
	}
	template<StringHash v>
	struct EnsureComplileTime{
		static constexpr StringHash value = v;
	};
	template<size_t N>
...
constexpr Utilities::StringHash operator "" _hash( const char* str, size_t size )
{
