...
#pragma region Free_Functions
		...
		// By const ref.
		template <class Return, typename... Param>
		auto map( Return( func )(const T&, Param...), Param... args ) noexcept -> optional<Return>
		{
			if ( !this->has_value() )
				return std::nullopt;
			return func( **this, std::forward<Param>( args )... );
		...
		template <typename... Param>
		auto map( void(func)( const T&, Param... ), Param... args ) noexcept -> void
		{
			if ( this->has_value() )
				func( **this, std::forward<Param>( args )... );
			return;
...
#pragma region Methods Lambda
		...
		auto map( const T& lambda, Param... args ) noexcept -> optional<decltype(lambda( **this, std::forward<Param>( args )... ))>
		{
			if ( !this->has_value() )
				return std::nullopt;
...
#pragma region Lambda
		template <class T, typename... Param>
		auto or_else( const T& lambda, Param... args )const noexcept
		{
			if ( !this->has_value() )
		...
		template <class T, typename... Param>
		inline auto or_else_this( const T& lambda, Param... args )const noexcept
		{
